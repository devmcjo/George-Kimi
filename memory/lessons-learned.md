...
상세 내용은 George-Claude의 `C:\George\memory\lessons-learned.md`를 참조하세요.

| 코드 | 제목 | 핵심 내용 |
|------|------|-----------|
| LL-A001 | 계획 단계에서 대상 코드베이스 수정 금지 | 계획 단계에서는 project 폴터에만 산출물 생성 |
| LL-A002 | 대규모 계획 수립 - 반복적 정교화 | 1차 거친 지도 → 2차 심층 → 3차 오탐 제거 |
| LL-A003 | 계획서-Redmine-스크립트 3자 정합성 | 1:1:1 대응 필수 |
...
| LL-G003 | Redmine 일감 담당자 자동 지정 필수 | `assigned_to_id: 871` (조명철) 필수 |
| LL-G004 | 일회성 스크립트 즉시 삭제 필수 | Git 커밋 전 체크리스트 100% 준수 |

---

## 🆕 George-Kimi 고유 학습 기록

> George-Kimi가 새로 발견한 교훈을 여기에 기록합니다.
> 코드: LL-K001부터 시작
...
**사건**: George-Kimi 초기 설정 완료 후 GitHub push를 자율적으로 수행함. mcJo가 "작업 완료 후 커밋은 스스로 진행하고, 푸시만 하지 말라"고 정정 지시.
...
### LL-K002: "접근 허용" = "묻지 않고 자율 진행" (George-Kimi, 2026-02-12)
...
**관련 파일**: `KIMI.md`, `memory/MEMORY.md`, `george-kimi_developer_profile.md`
...
### LL-K005: 작업 완료 후 자동 커밋 정책 (George-Kimi, 2026-02-12)
...
**커밋 규칙**:
- 메시지 형식: `[George] {타입}(#{레드마인번호}): {설명}`
- 타입: feat, fix, refactor, style, test, docs, chore 등
- 레드마인 번호: 해당 시 `(#번호)` 추가, 없으면 `(#0)` 사용
- **예시**: `[George] feat(#262550): 파일 로테이션 기능 추가`

**적용법**:
- 작업 완료 후 `git add` → `git commit` 자동 수행
- 변경 사항 없으면 커밋 생략
- push는 여전히 mcJo 전용 (금지 유지)

**관련 파일**: `KIMI.md`, `memory/MEMORY.md`, `AGENTS.md`

---

### LL-K007: 레드마인 댓글 작성 구분자 - "From.죠지" 사용 (George-Kimi, 2026-02-13)

**사건**: mcJo가 죠지와 자신이 동일한 Redmine 사용자 ID(871)를 공유하므로, 죠지가 스크립트로 작성한 댓글과 mcJo가 직접 작성한 댓글을 구분할 필요가 있다고 지시.

**교훈**:
| 상황 | 규칙 | 이유 |
|------|------|------|
| 죠지가 스크립트로 댓글 작성 | 댓글 상단에 **"From.죠지"** 추가 | 동일 ID(871) 사용으로 작성자 구분 필요 |
| mcJo가 직접 댓글 작성 | 구분자 없음 | 직접 작성임이 명확 |

**적용법**:
- Redmine API를 통해 댓글 작성 시: `notes` 필드에 `"From.죠지\n\n{실제 내용}"` 형식으로 작성
- LL-G003 (`assigned_to_id: 871`)과 병행 적용
- 구분자는 댓글 최상단에 위치

**관련 파일**: `redmine/README.md`, Redmine 스크립트 파일들

---

*이 파일은 지속적으로 업데이트됩니다. 교훈을 기록하여 같은 실수를 반복하지 않습니다.*
